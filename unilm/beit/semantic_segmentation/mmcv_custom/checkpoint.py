...
def load_checkpoint(model,
    ...
    if 'pos_embed' in state_dict and model.pos_embed != None:
    ...
    if 'blocks.0.attn.qkv.bias' in state_dict:
        block_keys = set(key.split('.')[1] for key in state_dict.keys() if 'blocks.' in key)
        num_blocks = len(block_keys)

        for i in range(num_blocks):
            qkv_bias = state_dict[f'blocks.{i}.attn.qkv.bias']
            q_bias, k_bias, v_bias = torch.split(qkv_bias, qkv_bias.size(0) // 3)
            state_dict[f'blocks.{i}.attn.q_bias'] = q_bias
            state_dict[f'blocks.{i}.attn.k_bias'] = k_bias
            state_dict[f'blocks.{i}.attn.v_bias'] = v_bias
